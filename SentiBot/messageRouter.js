...
class MessageRouter {
    constructor({ customerStore, dialogflowClient, projectId, sessionPath, customerRoom, operatorRoom }) {
        // Dialogflow client instance
        this.client = dialogflowClient;
        // Dialogflow project id
        this.projectId = projectId;
        // Dialogflow session path
    ...
    _routeCustomer(utterance, customer, customerId) {
        ...
        return this._sendUtteranceToOperator(utterance, customer)
            ...
            .then(responses => {
                console.log("Response with Sentiment Analysis");
    ...
    _checkOperatorMode(apiResponse) {
        ...
        let result = apiResponse.queryResult;
        if (result.sentimentAnalysisResult) {
            if (result.sentimentAnalysisResult.queryTextSentiment.score < -0.5) {
                operatorMode = true;
            }
        } else {
            console.log('No need to transfer based on sentiment Analysis Found');
        }
